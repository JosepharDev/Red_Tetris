...
function lockPiece(board, shape, px, py) {
...
function getStartPos(shape) {
  return { x: Math.floor((COLS - shape[0].length) / 2), y: 0 }
}

function buildDisplay(board, shape, px, py, ghostY, invisible) {
  const d = invisible
...
const EMPTY_BOARD = () => Array(ROWS).fill(null).map(() => Array(COLS).fill(0))
